...
>OPGG를 따라 만든 **개인프로젝트**
>
>index에서 간단한 다크모드, ~~register(login, logout)~~(현재 DB가 연동이 되어있지않아 로그인이 불가능합니다.), ~~닉네임을 검색하여 간단한 유저 정보와 최근 전적 검색~~(라이엇 계정 변경 정책에 따라 API가 변경되어 현재 조회가 불가능합니다. 빠른시일 내 수정하겠습니다.)
>
